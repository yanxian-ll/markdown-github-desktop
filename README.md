...
- 新增内置模板系统基础：LaTeX 基础论文、Markdown + Pandoc 论文、Beamer 演示文稿，可写入当前工作区子目录并自动打开主文件。
- 新增导出 profile 框架：PDF、DOCX、HTML、Beamer 默认配置入口，当前可直接触发 Pandoc 导出。
- 新增每日笔记和轻量快照：生成 `notes/daily/YYYY-MM-DD.md` 与 `.paper-notes/snapshots/<time>/manifest.md`。
- 新增 Zotero / Better BibTeX、DOI/ISBN/URL 导入、Hugo/Jekyll 发布、Mermaid/Plotly/TikZ、PDF 批注数据库等后续能力的 UI 与服务框架。
- `TODO.md` 和 `TESTING.md` 已按 v0.9.0 更新。
...
一个基于 Tauri 2 + Vue 3 + Vite 的跨平台学术写作、预览、批注和导出工作台。
...
1. 启动桌面版：
   ...
   npm run tauri:dev
   ```
2. 左侧“文档”点击打开按钮，选择一个包含 `.tex`、`.bib`、`figures/` 的论文文件夹。
