...
import type { BottomDockTab, LatexBuildResult } from '../types/app';
...
const emit = defineEmits<{
  ...
  updateActiveTab: [tab: BottomDockTab];
...
const combinedLog = computed(() => {
  const parts: string[] = [];
  const appLog = props.appLog?.trim();
  const latestBuildLog = props.latexResult?.log?.trim();
  if (appLog) parts.push(appLog);
  if (latestBuildLog && !appLog?.includes(latestBuildLog)) {
    parts.push(`[最新构建日志]\n${latestBuildLog}`);
  }
  return parts.join("\n\n") || '尚无运行日志。';
});
...
function switchTab(tab: BottomDockTab) {
  emit('updateActiveTab', tab);
  if (tab === 'evidence') emit('rebuildEvidence');
}
...
<template>
  <section class="bottom-panel bottom-dock-panel" aria-label="底部面板">
    <div class="bottom-panel-resize-grip" title="拖动调整底部面板高度" @mousedown="emit('resizeStart', $event)" />
    <div class="bottom-panel-header bottom-dock-header" title="双击关闭底部面板" @dblclick="onHeaderDblclick">
      <div class="bottom-panel-tabs bottom-dock-tabs">
        <button :class="{ active: activeTab === 'problems', error: allProblems.length }" @click="switchTab('problems')">问题 {{ allProblems.length }}</button>
        <button :class="{ active: activeTab === 'output' }" @click="switchTab('output')">输出</button>
        <button :class="{ active: activeTab === 'log' }" @click="switchTab('log')">日志</button>
        <span class="bottom-dock-tab-divider" />
        <button :class="{ active: activeTab === 'ai-chat' }" @click="switchTab('ai-chat')">AI 对话</button>
        <button :class="{ active: activeTab === 'evidence' }" @click="switchTab('evidence')">证据库 {{ indexStats.total }}</button>
        <button :class="{ active: activeTab === 'diff-review' }" @click="switchTab('diff-review')">Diff 审批 {{ patches.length }}</button>
        <button :class="{ active: activeTab === 'model' }" @click="switchTab('model')">模型</button>
      </div>
      <div class="ai-dock-header-meta">
        <span v-if="['ai-chat','evidence','diff-review','model'].includes(activeTab)" class="ai-grounding-pill">{{ groundingLabel }}</span>
        <button class="toolbar-icon" title="关闭底部面板" @click="emit('close')">×</button>
      </div>
    </div>

    <div class="bottom-panel-body bottom-dock-body">
      <div v-if="activeTab === 'problems'" class="problem-list">
      ...
      <pre v-else-if="activeTab === 'output'" class="build-log">{{ latexResult?.command || '尚未构建。' }}</pre>
      <pre v-else-if="activeTab === 'log'" class="build-log">{{ combinedLog }}</pre>

      <div v-else-if="activeTab === 'ai-chat'" class="ai-chat-layout">
      ...
      <div v-else-if="activeTab === 'evidence'" class="ai-evidence-layout">
      ...
      <div v-else-if="activeTab === 'diff-review'" class="ai-diff-layout">
